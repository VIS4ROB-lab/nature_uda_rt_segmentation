...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', type=str, default='', help='Resume training from given checkpoint')
    parser.add_argument(
    ...
    if args.config is not None:
        ...
        if len(args.resume) == 0:
            ...
            unique_name = args.resume
            child_cfg = {
                '_base_': args.config.replace('configs', '../..'),
                'name': unique_name,
                'work_dir': os.path.join('work_dirs', exp_name, unique_name),
                'git_rev': get_git_hash(),
                'resume_from': os.path.join('work_dirs', exp_name, args.resume, 'latest.pth')
            }
            cfg_out_file = f"{GEN_CONFIG_DIR}/{exp_name}/{child_cfg['name']}.json"
            cfg.resume_from = os.path.join('work_dirs', exp_name, args.resume, 'latest.pth')
